...
Follow always these general principles:
...
- ALWAYS add tags to each created resource, unless the resource itself does not support them. Use descriptive and meaningful key-values for the tags referring to the purpose and possible usage of the resource.
...
The below patterns can be applied to any Terraform code suitable for migration but migration must be explicitly requested as part of the prompt. **DO NOT ATTEMPT ANY MIGRATION/REFACTORING STEPS** if not explicitily asked.
